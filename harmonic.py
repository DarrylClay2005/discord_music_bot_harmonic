...
async def ensure_voice_connection(guild, channel_id):
    ...
    try:
        ...
        # Bypass Discord's native Stage Channel Mute
                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO harmonic_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'harmonic', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
                                elif loop_mode == 'song':
                                    await cur.execute("SELECT COALESCE(MIN(id), 0) FROM harmonic_queue WHERE guild_id = %s", (payload.player.guild.id,))
                                    min_row = await cur.fetchone()
                                    new_id = ((min_row[0] if min_row else 0) or 0) - 1
                                    await cur.execute("INSERT INTO harmonic_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'harmonic', %s, %s, %s)", (new_id, payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
        except Exception:
            pass
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            ...
                            await cur.execute("SELECT MIN(id) as m_id FROM harmonic_queue WHERE guild_id = %s", (guild.id,))
                            min_res = await cur.fetchone()
                            new_id = (min_res['m_id'] or 0) - 1
                            await cur.execute("INSERT INTO harmonic_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'harmonic', %s, %s, %s)", (new_id, guild.id, orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id))
...
async def init_playlist_db():
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute('''
                    CREATE TABLE IF NOT EXISTS harmonic_active_playlists (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, known_track_count INT DEFAULT 0, requester_id BIGINT, PRIMARY KEY (guild_id, bot_name))
                ''')
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT guild_id, playlist_url, known_track_count, requester_id FROM harmonic_active_playlists WHERE bot_name = 'harmonic'")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if t_url: await cur.execute("INSERT INTO harmonic_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'harmonic', %s, %s, %s)", (guild_id, t_url, t_title, req_id))
...
async def zombie_reaper_loop():
    for guild in bot.guilds:
        ...
        if vc and not getattr(vc, 'playing', False) and not getattr(vc, 'paused', False):
            # Check if there's an active queue but it's completely dead
            try:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                    ...
                    if guild.id in playback_tracking:
                        ...
                        if now - track_info.get('start_time', 0) > 10:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("SELECT id FROM harmonic_queue WHERE guild_id = %s LIMIT 1", (guild.id,))
